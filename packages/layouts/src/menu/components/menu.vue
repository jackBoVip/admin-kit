...
import { Ellipsis } from '@admin-core/icons';
...
const getSlot = computed(() => {
  ...
  // 仅在 horizontal（需要计算 “更多” 溢出）时做深度扁平化，避免每次渲染都递归展开 subTree
  const originalSlot = (props.mode === 'horizontal'
    ? flattedChildren(defaultSlots, { traverseSubTree: false })
    : defaultSlots) as VNodeArrayChildren;
  const slotDefault =
    sliceIndex.value === -1
      ? originalSlot
...
<template>
  ...
  >
    <template v-if="mode === 'horizontal' && getSlot.showSlotMore">
      <template v-for="(item, idx) in getSlot.slotDefault" :key="(item as any)?.key ?? idx">
        <component :is="item" />
      </template>
      <SubMenu is-sub-menu-more path="sub-menu-more">
        ...
        <template v-for="(item, idx) in getSlot.slotMore" :key="(item as any)?.key ?? idx">
          <component :is="item" />
        </template>
      </SubMenu>
    </template>
