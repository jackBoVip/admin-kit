...
// 注意：请从 shared/utils 导入（shared package exports 不提供 global-state 子路径）
      ...
      <div :class="cn('relative flex-1 overflow-y-auto p-3', contentClass)">
        ...
        <AlertDialogDescription>
          <div class="m-4 min-h-[30px]">
            <AdminRenderContent :content="content" render-br />
          </div>
          <AdminLoading v-if="loading && contentMasking" :spinning="loading" />
        </AlertDialogDescription>
        <div
          class="flex items-center justify-end gap-x-2"
          :class="`justify-${buttonAlign}`"
        >
          <AdminRenderContent :content="footer" />
          <AlertDialogCancel v-if="showCancel" as-child>
            <component
              :is="components.DefaultButton || AdminButton"
              :disabled="loading"
              variant="ghost"
              @click="handleCancel"
            >
          ...
          <AlertDialogAction as-child>
            <component
              :is="components.PrimaryButton || AdminButton"
              :loading="loading"
              @click="handleConfirm"
            >
