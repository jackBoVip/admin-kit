...
import type { ValueType, AdminButtonGroupProps } from './button';

import { computed, ref, watch } from 'vue';

import { Circle, CircleCheckBig, LoaderCircle } from '@admin-core/icons';
import { cn, isFunction } from '@admin-core/shared/utils';

import { objectOmit } from '@vueuse/core';

import { AdminRenderContent } from '../render-content';
import AdminButtonGroup from './button-group.vue';
import Button from './button.vue';

const props = withDefaults(defineProps<AdminButtonGroupProps>(), {
  gap: 0,
  multiple: false,
  showIcon: true,
  size: 'middle',
  ...
  >
    ...
    >
      ...
      <slot name="option" :label="btn.label" :value="btn.value" :data="btn">
        <AdminRenderContent :content="btn.label" />
      </slot>
    </Button>
  </AdminButtonGroup>
</template>
